...
export class PostsService {
    ...
    async getAllPosts() {
        try {
            const allPost = await this.postModel.find().populate('authorId', 'username');
    ...
    async getPostById(id: string) {
        try {
            const post = await this.postModel.findById(id).populate('authorId', 'username');
            if (!post) {
    ...
    async createPost(createPostDto: CreatePostDto, userId: string) {
        try {
            const checkExistTitle = await this.postModel.findOne({ title: createPostDto.title });
            if (checkExistTitle) {
                throw new HttpException('Title already exist', 400);
            }
            const slugName = configSlug.convertToSlug(createPostDto.title);
            const newPost = new this.postModel({ ...createPostDto, slugName, authorId: userId });
            await newPost.save();
