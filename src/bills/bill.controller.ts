...
import { permissionErrorResponse, serverErrorResponse, tokenErrorResponse } from '../global/api-responses';
...
export class BillController {
	...
	@tokenErrorResponse
	@ApiResponse({
		status: 200, description: 'get all bills successfully', schema: {
	...
	// owner user
	@ApiResponse({
		status: 200, description: 'checkout bill successfully', schema: {
			example: new responseData(null, 200, 'checkout bill successfully')
		}
	})
	@ApiResponse({
		status: 404, description: 'bill is not exist', schema: {
	...
	// owner user
	@ApiResponse({
		status: 200, description: 'get all dishes in cart successfully', schema: {
			...
			], 200, 'get all dishes in cart successfully')
		}
	})
	@Get('/cart')
	@UseGuards(AuthGuard)
	...
	@ApiResponse({
		status: 200, description: 'reset cart successfully', schema: {
	...
	// add dish to cart
	@ApiResponse({
		status: 200, description: 'add dish to cart successfully', schema: {
			example: new responseData(null, 200, 'add dish to cart successfully')
		}
	})
	@ApiResponse({
		status: 400, description: 'dishId is not valid', schema: {
	...
	@ApiResponse({
		status: 200, description: 'subtract dish from cart successfully', schema: {
			example: new responseData(null, 200, 'subtract dish from cart successfully')
		}
	})
	@ApiResponse({
		status: 400, description: 'dishId is not valid', schema: {
	...
	@ApiResponse({
		status: 200, description: 'remove dish from cart successfully', schema: {
			example: new responseData(null, 200, 'remove dish from cart successfully')
		}
	})
	@ApiResponse({
		status: 400, description: 'dishId is not valid', schema: {
